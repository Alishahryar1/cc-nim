...
async def test_fusion_mode():
    ...
    def provider_getter(provider_id):
        if provider_id == "mistral": return provider_panel1
        if provider_id == "deepseek": return provider_panel2
    ...
    assert "Synthesized Result" in full_response
...
async def test_named_fusion_panel():
    ...
    def provider_getter(provider_id):
        if provider_id == "mistral": return MockStreamProvider(None, "m1")
        if provider_id == "deepseek": return MockStreamProvider(None, "m2")
        return provider_judge
    ...
    assert "Named Panel Success" in full_response

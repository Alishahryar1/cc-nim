...
def test_key_rotation_logic():
    config = ProviderConfig(api_key="key1", api_keys=["key1", "key2", "key3"])

    provider = MockOpenAIProvider(
        config, provider_name="test", base_url="http://localhost", api_key="key1"
    )
...
def test_single_key_logic():
    config = ProviderConfig(api_key="key1", api_keys=["key1"])

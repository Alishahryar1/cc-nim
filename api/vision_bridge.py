...
class VisionBridge:
    ...
    async def bridge_vision(
        self, request_data: MessagesRequest, target_model_ref: str
    ) -> AsyncIterator[str]:
        ...
        for msg in request_data.messages:
            ...
            new_content = []
            for block in msg.content:
                is_image = getattr(block, "type", None) == "image" or (
                    isinstance(block, dict) and block.get("type") == "image"
                )
                if is_image:
        ...
        candidates = self.proxy_service._model_router.resolve_candidates(
            target_model_ref
        )
    ...
    def _select_best_vision_expert(self) -> str:
        ...
        if not models:
            return "nvidia_nim/nvidia/llama-3.2-90b-vision-instruct"
            ...
            messages=[
                Message(
                    ...
                    content=[
                        ...
                        ContentBlockText(
                            type="text",
                            text="Describe this image in detail for a text-only AI model. Focus on layout, text, and key visual elements.",
                        ),
                    ],
                )
            ],

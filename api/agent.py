...
class AgentEngine:
    ...
    async def run_agent(self, request_data: MessagesRequest, planner_model: str, executor_model: str) -> AsyncIterator[str]:
        """Run an autonomous Plan-and-Execute loop."""
        logger.info("Starting Plan-and-Execute Agent: planner={}, executor={}", planner_model, executor_model)
        ...
        yield sse.emit_thinking_delta(f"Planning execution strategy using {planner_model}...\n")
        ...
        planner_request = request_data.model_copy(update={
            "messages": [Message(role="user", content=plan_prompt)]
        })

        plan_raw = await self.proxy_service._orchestrator._get_full_response(planner_model, planner_request)

        try:
            plan_json = json.loads(re.search(r'\{.*\}', plan_raw, re.DOTALL).group(0))
            tasks = plan_json.get("tasks", [request_data.messages[-1].content])
        except:
            logger.warning("Agent Planning failed to produce JSON, using raw request as single task.")
        ...
        for i, task in enumerate(tasks):
            ...
            exec_request = request_data.model_copy(update={
                "messages": [Message(role="user", content=exec_prompt)]
            })

            res = await self.proxy_service._orchestrator._get_full_response(executor_model, exec_request)
            results.append(f"Result {i+1}: {res}")

        # STAGE 3: Final Aggregation
        yield sse.emit_thinking_delta("Execution finished. Synthesizing final response...\n")
        ...
        agg_request = request_data.model_copy(update={
            "messages": request_data.messages + [Message(role="user", content=agg_prompt)]
        })

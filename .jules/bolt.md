...
## 2026-05-13 - Avoid Eager Dictionary Instantiation with .get()
**Learning:** Using `dict.get("key", {})` inside high-frequency loops (like SSE stream parsing) is inefficient because the empty dictionary `{}` is instantiated on every single iteration before `.get()` is called, causing unnecessary allocations and garbage collection overhead. Even worse, if the intention is only to check `if isinstance(val, dict):`, allocating a fallback dictionary is completely pointless.
**Action:** Replace `val = event.get("key", {})` with `val = event.get("key")` when the result is only used in a type check (`isinstance(val, dict)` evaluates to `False` for `None`). When a dictionary fallback is strictly required, use `val = event.get("key"); if val is None: val = {}` to avoid the allocation overhead on the hot path.

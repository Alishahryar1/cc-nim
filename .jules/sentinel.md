...
## 2026-05-13 - FastAPI Security Middlewares
**Vulnerability:** Missing explicit CORS restrictions and DNS rebinding protections.
**Learning:** Default FastAPI configurations are permissive by default (`*` origins, any host header). While useful for local development, they must be hardened or made configurable for production-ready proxies.
**Prevention:** Always introduce `CORSMiddleware` and `TrustedHostMiddleware` with environment-driven configurations (`CORS_ORIGINS`, `ALLOWED_HOSTS`) to restrict access. Ensure `TrustedHostMiddleware` is added last to intercept requests first.

...
## 2026-05-27 - Adding Configurable CORS and TrustedHost Middlewares
**Vulnerability:** Overly permissive CORS and missing host header validation.
**Learning:** FastAPI applications exposed to the network must implement CORS and TrustedHost middlewares to prevent Cross-Site Request Forgery (CSRF) and DNS rebinding attacks, respectively.
**Prevention:** Added `CORSMiddleware` and `TrustedHostMiddleware` driven by `Settings.cors_origins` and `Settings.allowed_hosts` with safe defaults. `allow_credentials` is conditionally disabled if origins contain `*` to satisfy Starlette's safety checks.

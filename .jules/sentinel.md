...
## 2026-05-18 - Restrict CORS Configuration
**Vulnerability:** Overly permissive CORS configuration allowing requests from any origin.
**Learning:** Local proxies should restrict CORS to loopback interfaces (`localhost`, `127.0.0.1`, `[::1]`) to prevent browser-based attacks from malicious external websites targeting the local API.
**Prevention:** Always configure `CORSMiddleware` with `allow_origin_regex` to explicitly restrict cross-origin access to loopback interfaces.

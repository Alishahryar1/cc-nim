...
class RTKCompressor:
    ...
    def compress(self, command: str, output: str) -> str:
        ...
        if self.verbosity >= 2:
            logger.debug(f"RTK optimized '{command}': {original_lines} -> {new_lines} lines")
    ...
    def _truncate_horizontal_bloat(self, output: str) -> str:
        ...
        for line in lines:
            if len(line) > max_line_len:
                ...
                half = max_line_len // 2
                processed.append(f"{line[:half]} ... [RTK Line Snip] ... {line[-half:]}")
            else:
    ...
    def _smart_test_compress(self, output: str) -> str:
        ...
        for line in lines:
            ...
            if any(err in upper_line for err in ["FAIL", "ERROR", "TRACED", "EXCEPTION", "FAILED", "ERRORS"]):
    ...
    def _smart_diff_compress(self, output: str) -> str:
        ...
        compressed = []
        for line in lines:
            if line.startswith(("diff --git", "@@", "+", "-")) and not line.startswith(("+++ b/", "--- a/")):
    ...
    def _smart_status_compress(self, output: str) -> str:
        ...
        for line in lines:
            clean = line.strip()
            if clean.startswith(("modified:", "new file:", "deleted:", "??", "changes to be committed:")):

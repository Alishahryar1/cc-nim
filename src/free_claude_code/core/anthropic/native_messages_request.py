...
def _resolve_budget_tokens(
    thinking_cfg: dict[str, Any], raw_request: Any = None
) -> int:
    budget_tokens = thinking_cfg.get("budget_tokens")
    ...
    if not effort and raw_request:
        if hasattr(raw_request, "output_config") and isinstance(
            raw_request.output_config, dict
        ):
            effort = raw_request.output_config.get("effort")
        if (
            not effort
            and hasattr(raw_request, "extra_body")
            and isinstance(raw_request.extra_body, dict)
        ):
            effort = raw_request.extra_body.get("effort")

    if isinstance(effort, str) and effort.lower() in EFFORT_TO_BUDGET_TOKENS:
        return EFFORT_TO_BUDGET_TOKENS[effort.lower()]
    ...
    *,
    ...
    if "thinking" in body:
        ...
        if thinking_enabled and isinstance(thinking_cfg, dict):
            ...
            thinking_payload["budget_tokens"] = _resolve_budget_tokens(
                thinking_cfg, raw_request=request
            )

...
class LlamaCPPProvider:
    def __init__(self, base_url: str | None = None, default_model: str = "default"):
    ...
    async def execute(
        ...
        **kwargs,
        ...
        logger.debug(
            f"Sending request to LlamaCPP provider at {endpoint} with model {target_model}"
        )
                ...
                async def response_generator():
                    async with client.stream(
                        ...
                        if response.status_code != 200:
                            ...
                            raise RuntimeError(
                                f"LlamaCPP API error [{response.status_code}]: {error_text.decode('utf-8', errors='ignore')}"
                            )

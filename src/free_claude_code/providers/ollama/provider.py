...
class OllamaProvider:
    def __init__(self, base_url: str = None, default_model: str = "llama3.1"):
    ...
    async def execute(self, messages: list[dict], model: str = None, tools: list[dict] = None, stream: bool = False, **kwargs):
        ...
        logger.debug(f"Sending request to Ollama provider at {endpoint} with model {target_model}")
                ...
                async def response_generator():
                    async with client.stream("POST", endpoint, json=payload, headers=headers) as response:
                        if response.status_code != 200:
                            ...
                            raise RuntimeError(f"Ollama API error [{response.status_code}]: {error_text.decode('utf-8', errors='ignore')}")
